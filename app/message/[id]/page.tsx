...
import { checkMessageExists } from "@/lib/api";
...
export default function Page() {
  ...
  useEffect(() => {
    const check = async () => {
      try {
        console.log('Checking message exists for slug:', id);
        const res = await checkMessageExists(id);
        console.log('checkMessageExists response:', res);
        ...
        <div className="relative h-full flex flex-col items-center justify-start overflow-hidden">
          ...
          <main className="flex flex-col items-center h-full text-center px-6 max-w-2xl z-10">
            <h1 className="text-4xl md:text-5xl font-normal text-[#FFC758] mb-4">
              You Don&apos;t Yet Have a Message
            </h1>
            <p className="text-base text-[#FFFFFF] mb-6">
              No message has been sent to this link yet. Check back later!
            </p>
          </main>
